//
//  MKMPLoRaAppSettingController.m
...
@interface MKMPLoRaAppSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKMPLoRaAppSettingModel *dataModel;

@end

@implementation MKMPLoRaAppSettingController

- (void)dealloc {
    NSLog(@"MKMPLoRaAppSettingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MP", @"MKMPLoRaAppSettingController", @"mp_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMPLoRaAppSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMPLoRaAppSettingModel alloc] init];
    }
    return _dataModel;
}
