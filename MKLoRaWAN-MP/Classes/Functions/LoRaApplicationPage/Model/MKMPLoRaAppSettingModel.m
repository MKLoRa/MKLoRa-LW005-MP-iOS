...
@interface MKMPLoRaAppSettingModel ()
...
@implementation MKMPLoRaAppSettingModel
...
- (BOOL)readTimeSyncInterval {
    ...
    [MKMPInterface mp_readLorawanTimeSyncIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimeSyncInterval {
    __block BOOL success = NO;
    [MKMPInterface mp_configTimeSyncInterval:[self.timeSyncInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readNetworkCheckInterval {
    __block BOOL success = NO;
    [MKMPInterface mp_readLorawanNetworkCheckIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNetworkCheckInterval {
    __block BOOL success = NO;
    [MKMPInterface mp_configLorawanNetworkCheckInterval:[self.checkInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
