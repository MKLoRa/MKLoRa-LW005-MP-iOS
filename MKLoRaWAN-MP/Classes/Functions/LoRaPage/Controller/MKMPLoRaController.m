...
@interface MKMPLoRaController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKMPLoRaPageModel *dataModel;

@end

@implementation MKMPLoRaController

- (void)dealloc {
    NSLog(@"MKMPLoRaController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 1) {
        //Connection Settings
        MKMPLoRaSettingController *vc = [[MKMPLoRaSettingController alloc] init];
...
- (MKMPLoRaPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMPLoRaPageModel alloc] init];
    }
    return _dataModel;
}
