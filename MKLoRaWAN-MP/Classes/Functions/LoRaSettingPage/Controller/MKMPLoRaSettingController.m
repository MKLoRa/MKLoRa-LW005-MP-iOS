...
@interface MKMPLoRaSettingController ()<UITableViewDelegate,
...
MKMPLoRaSettingAccountCellDelegate>
...
@property (nonatomic, strong)MKMPLoRaSettingModel *dataModel;

@end

@implementation MKMPLoRaSettingController

- (void)dealloc {
    NSLog(@"MKMPLoRaSettingController销毁");
}
...
- (void)rightButtonMethod {
    ...
    if (!ValidStr([MKMPUserLoginManager shared].password)) {
        //当前未登录
        @weakify(self);
    ...
    //登录过
    [self login:[MKMPUserLoginManager shared].isHome username:[MKMPUserLoginManager shared].username password:[MKMPUserLoginManager shared].password];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 7) {
        //Account
        return ((self.dataModel.configModel.supportServerPlatform && self.dataModel.platform == 1 && ValidStr([MKMPUserLoginManager shared].password)) ? self.section6List.count : 0);
    }
    if (section == 8) {
        //Gateway EUI(Gateway ID)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        //mokoDevEUI
        MKMPDevEUICell *cell = [MKMPDevEUICell initCellWithTableView:tableView];
    ...
    if (indexPath.section == 7) {
        //Account
        MKMPLoRaSettingAccountCell *cell = [MKMPLoRaSettingAccountCell initCellWithTableView:tableView];
...
#pragma mark - MKMPLoRaSettingAccountCellDelegate
- (void)mp_loRaSettingAccountCell_logoutBtnPressed {
    MKIoTCloudExitAccountAlert *alert = [[MKIoTCloudExitAccountAlert alloc] init];
    BOOL isHome = [MKMPUserLoginManager shared].isHome;
    NSString *username = [MKMPUserLoginManager shared].username;
    [alert showWithAccount:username completeBlock:^{
        [[MKMPUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:@""];
...
- (void)login:(BOOL)isHome username:(NSString *)username password:(NSString *)password {
    ...
    [[MKIoTLoginService share] loginWithUsername:username password:password isHome:isHome sucBlock:^(id returnData) {
        ...
        [[MKMPUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:password];
...
- (void)addDeviceToCloud:(NSString *)token {
    ...
    MKMPCreateLoRaDeviceModel *createModel = [[MKMPCreateLoRaDeviceModel alloc] init];
    createModel.macAddress = [MKMPConnectModel shared].macAddress;
    createModel.isHome = [MKMPUserLoginManager shared].isHome;
    createModel.gwId = self.dataModel.gatewayEUI;
    createModel.region = self.dataModel.region;
    createModel.username = [MKMPUserLoginManager shared].username;
    [[MKMPNetworkService share] addLoRaDeviceToCloud:createModel token:token sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
        [self saveDataToDevice];
    } failBlock:^(NSError *error) {
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    MKMPDevEUICellModel *cellModel = [[MKMPDevEUICellModel alloc] init];
    NSString *macAddress = [[MKMPConnectModel shared].macAddress stringByReplacingOccurrencesOfString:@":" withString:@""];
...
- (void)loadSection6Datas {
    MKMPLoRaSettingAccountCellModel *cellModel = [[MKMPLoRaSettingAccountCellModel alloc] init];
    cellModel.account = [MKMPUserLoginManager shared].username;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MP", @"MKMPLoRaSettingController", @"mp_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKMPLoRaSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKMPLoRaSettingModel alloc] init];
    }
    return _dataModel;
}
