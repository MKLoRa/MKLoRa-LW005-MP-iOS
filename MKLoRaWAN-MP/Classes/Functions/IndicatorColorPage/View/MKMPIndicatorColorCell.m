...
@implementation MKMPIndicatorColorCellModel
@end

@interface MKMPIndicatorColorCell ()
...
@implementation MKMPIndicatorColorCell

+ (MKMPIndicatorColorCell *)initCellWithTableView:(UITableView *)tableView {
    MKMPIndicatorColorCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMPIndicatorColorCellIdenty"];
    if (!cell) {
        cell = [[MKMPIndicatorColorCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKMPIndicatorColorCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKMPIndicatorColorCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMPIndicatorColorCellModel.class]) {
        return;
    }
...
- (MKTextField *)textField {
    if (!_textField) {
        ...
        _textField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mp_ledColorChanged:index:)]) {
                [self.delegate mp_ledColorChanged:text index:self.dataModel.index];
            }
        };
