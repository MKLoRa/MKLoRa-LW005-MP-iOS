...
static NSString *const localPasswordKey = @"mk_mp_passwordKey";

static CGFloat const searchButtonHeight = 40.f;

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKMPScanController ()<UITableViewDelegate,
UITableViewDataSource,
mk_mp_centralManagerScanDelegate,
MKMPScanPageCellDelegate,
MKSearchButtonDelegate,
MKMPTabBarControllerDelegate>
...
@implementation MKMPScanController

- (void)dealloc {
    NSLog(@"MKMPScanController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKMPCentralManager shared] stopScan];
    [MKMPCentralManager removeFromCentralList];
}
...
- (void)viewDidLoad {
    ...
    [MKMPCentralManager shared].delegate = self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKMPScanPageCell *cell = [MKMPScanPageCell initCellWithTableView:self.tableView];
...
#pragma mark - mk_mp_centralManagerScanDelegate
- (void)mk_mp_receiveDevice:(NSDictionary *)deviceModel {
    MKMPScanPageModel *model = [MKMPScanPageModel mk_modelWithJSON:deviceModel];
...
#pragma mark - MKMPScanPageCellDelegate
- (void)mp_scanCellConnectButtonPressed:(NSInteger)index {
    [self connectDeviceWithModel:self.dataList[index]];
}

#pragma mark - MKMPTabBarControllerDelegate
- (void)mk_mp_needResetScanDelegate:(BOOL)need {
    if (need) {
        [MKMPCentralManager shared].delegate = self;
    }
...
- (void)scanTimerRun{
    ...
    [[MKMPCentralManager shared] startScan];
...
- (void)updateDataWithScanModel:(MKMPScanPageModel *)scanDataModel{
 ...
 */
- (void)filterTrackerDataWithSearchName:(MKMPScanPageModel *)scanDataModel{
    if (scanDataModel.rssi < self.buttonModel.searchRssi) {
        return;
    }
...
- (void)processTrackerData:(MKMPScanPageModel *)scanDataModel{
    //查看数据源中是否已经存在相关设备
 ...
 */
- (void)dataNoExistDataSource:(MKMPScanPageModel *)scanDataModel{
 ...
 */
- (void)dataExistDataSource:(MKMPScanPageModel *)scanDataModel {
    NSInteger currentIndex = 0;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKMPScanPageModel *dataModel = self.dataList[i];
        if ([dataModel.macAddress isEqualToString:scanDataModel.macAddress]) {
            currentIndex = i;
            break;
        }
    }
    MKMPScanPageModel *dataModel = self.dataList[currentIndex];
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKMPScanPageModel *)scanDataModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKMPCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDeviceWithDataModel:(MKMPScanPageModel *)scanDataModel {
    if (scanDataModel.needPassword) {
    ...
    [[MKMPConnectModel shared] connectDevice:scanDataModel.peripheral password:(scanDataModel.needPassword ? self.passwordField.text : @"") macAddress:scanDataModel.macAddress sucBlock:^{
...
- (void)pushTabBarPage {
    MKMPTabBarController *vc = [[MKMPTabBarController alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-MP", @"MKMPScanController", @"mp_scanRightAboutIcon.png") forState:UIControlStateNormal];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKLoRaWAN-MP", @"MKMPScanController", @"mp_scan_refreshIcon.png");
    }
    return _refreshIcon;
}
