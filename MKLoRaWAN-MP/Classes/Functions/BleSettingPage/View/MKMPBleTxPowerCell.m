...
@implementation MKMPBleTxPowerCellModel
@end

@interface MKMPBleTxPowerCell ()
...
@implementation MKMPBleTxPowerCell

+ (MKMPBleTxPowerCell *)initCellWithTableView:(UITableView *)tableView {
    MKMPBleTxPowerCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKMPBleTxPowerCellIdenty"];
    if (!cell) {
        cell = [[MKMPBleTxPowerCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKMPBleTxPowerCellIdenty"];
    }
    return cell;
}
...
- (void)txPowerSliderValueChanged {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_mp_txPowerValueChanged:)]) {
        [self.delegate mk_mp_txPowerValueChanged:(NSInteger)self.txPowerSlider.value];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKMPBleTxPowerCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKMPBleTxPowerCellModel.class]) {
        return;
    }
